...
def train_SemiSelf():
    ...
    model = SemiSelfClassifier(n_classes=[torch.unique(targets1).size(0), torch.unique(true_targets2).size(0)], img_h=img_h, img_w=img_w, n_decov=args.ndeconv, channels=args.channels, triplet=args.triplet)
    if args.retrain:
