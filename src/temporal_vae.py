...
class TensorTransforms():
    ...
    def __call__(self, tensors):
        ...
        return torch.Tensor(trans_tensors)
...
class rotate():
    ...
    def __call__(self, tensor:
        if random.random <= self.p:
            return tensor.rot90(random.randint(1,3), [1,2])
        else:
            return tensor
...
def train_TDAE_VAE_fullsuper_disentangle():
    args = argparses()
    img_w, img_h, out_source_dpath, data_path, ex = get_outputpath()
    out_source_dpath += '_Rec'
    if args.semi:
        out_source_dpath += '_SemiSelf'
    ...
    for epoch in range(n_epochs):
        ...
        for iters, (idx, p_idx, n_idx, target, sub_target) in enumerate(train_loader):
            ...
            loss_classifier_main.backward(retain_graph=True)
            loss_classifier_sub.backward(retain_graph=True)
            if args.use_pseudo and (epoch+1 > 100):
        ...
        if (epoch + 1) % args.step == 0:
            with torch.no_grad():
                ...
                for v_i, (idx, p_idx, n_idx, target, sub_target) in enumerate(val_loader):
                    ...
                    if args.use_pseudo (epoch+1 > 100):
                ...
                if ((epoch + 1) > T1) and (best_semi_loss > val_loss_dict['loss/val_all']):
                    ...
                    torch.save(model.state_dict(), '{}/TDAE_test_semi_bestparam.json'.format(out_param_dpath))
        ...
        for iters, (idx, p_idx, n_idx, target, sub_target) in enumerate(train_loader):
            ...
            if args.triplet:
                if args.single_triplet:
                    ...
                    if args.dual:
                        loss_triplet = weight_dict['triplet'] * (criterion_triplet(z2, p_z2, n_z2) + criterion_triplet(z1, p_z1, n_z1))
                    else:
                        loss_triplet = weight_dict['triplet'] * criterion_triplet(z2, p_z2, n_z2)
                    loss_triplet.backward(retain_graph=True)
            else:
            ...
            if args.adv > 0:
                ...
                if args.use_pseudo and (epoch+1 > T1):
                    if args.dual:
                        loss_pseudo_label1 = semi_alpha * weight_dict['adv_sub'] * criterion_classifier(sub_preds_adv_no_grad.to(device), pseudo_label1[idx].to(device))
                        loss_pseudo_label2 = semi_alpha * weight_dict['adv_main'] * criterion_classifier(preds_adv_no_grad.to(device), pseudo_label[idx].to(device))
        ...
        if (epoch + 1) % args.step == 0:
            with torch.no_grad():
                ...
                for v_i, (idx, p_idx, n_idx, target, sub_target) in enumerate(val_loader):
                    ...
                    val_loss = val_loss_classifier_main + val_loss_classifier_sub + val_loss_adv_main + val_loss_classifier_sub + val_loss_adv_sub
                    # val_loss = val_loss_classifier_main + val_loss_classifier_sub + val_loss_adv_main + val_loss_no_grad_main + val_loss_classifier_sub + val_loss_adv_sub + val_loss_no_grad_sub + val_loss_triplet + val_loss_pseudo_label + val_loss_pseudo_classifier
...
def output_imgexaple():
    args = argparses()
    img_w, img_h, out_source_dpath, data_path, ex = get_outputpath()
    src, targets1, targets2, idxs, seq_ids = get_triplet_flatted_data_with_idx(data_path, label_decomp=args.labeldecomp)
    for u1, u2 in itertools.product(np.unique(targets1), np.unique(targets2)):
        ...
        for img in cat_src:
            ...
            print(img.shape)
            img = img[:,:,::-1]
            cv2.imwrite('./reports/test_loc{}_level{}_iter{}.png'.format(u1, u2, iters), img*255)
            iters += 1
            if iters >= 10:
                break
