...
class TDAE_VAE_fullsuper(nn.Module):
    def __init__(self, n_classes, ksize=3, img_w=256, img_h=256, channels=[3, 16, 32, 64, 128], n_decov=2, latent_dim=256, base_net='res', triplet=False):
        super(TDAE_VAE_fullsuper, self).__init__()
        self.img_h, self.img_w = img_h, img_w
        self.channels = channels
        self.latent_dim = latent_dim
        self.triplet = triplet
    ...
    def forward(self, input, latent=False):
        ...
        rec = self.decode(z1, z2)
        if self.triplet:
            return classifier_preds[0], classifier_preds[1], rec, z1, z2, mu1, mu2, logvar1, logvar2

        return classifier_preds[0], classifier_preds[1], rec, mu1, mu2, logvar1, logvar2
