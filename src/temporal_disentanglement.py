...
def test_TDAE():
    ...
    with torch.no_grad():
        ...
        p0 = p[Y2[0]==0]
        p1 = p[Y2[0]==1]
        # print(np.sum(p==Y2[0]), np.sum(p==Y2[0])/len(Y2[0]))
        print('train-------------------------------')
        print(np.sum(Y2[0]==0), np.sum(Y2[0]==1))
        print(np.sum(p0==0), np.sum(p1==1))
        print(np.sum(p0==1), np.sum(p1==0))
        print(score_reg)
        ...
        # print(np.sum(p==1))
        print('val-------------------------------')
        print(np.sum(Y2[1]==0), np.sum(Y2[1]==1))
        print(np.sum(p0==0), np.sum(p1==1))
        print(np.sum(p0==1), np.sum(p1==0))
        print(score_reg)
        score_reg = logreg.score(X2[2], Y2[2])
        l = logreg.predict_proba(X2[2])
        p = np.argmax(l, axis=1)
        p0 = p[Y2[2]==0]
        p1 = p[Y2[2]==1]
        print('test-------------------------------')
        print(np.sum(Y2[2]==0), np.sum(Y2[2]==1))
        print(np.sum(p0==0), np.sum(p1==1))
        print(np.sum(p0==1), np.sum(p1==0))
        print(score_reg)
        return
